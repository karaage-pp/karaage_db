...
color = [cm.gist_rainbow(i / float(20))for i in range(20)]
...
class Objects(object):
    ...
    def _visulaize_marker(self, sql_data):
        ...
        for i, _ in sql_data.iterrows():
            #for text
            marker = Marker()
            objects_pos = list(sql_data.ix[i, ["position_x", "position_y", "position_z", "generic_id_0", "generic_score_0","generic_name_0", "specific_id_0"]])
            print objects_pos[6]
            if objects_pos[6] is not 12:
                continue
            marker.id = i #"object_{0}".format(i)
            marker.header.frame_id = "/map"
            marker.type = marker.TEXT_VIEW_FACING #marker.SPHERE
            ...
            marker.color.r = color[objects_pos[6]][0]
            marker.color.g = color[objects_pos[6]][1]
            marker.color.b = color[objects_pos[6]][2]
            marker.text = str(objects_pos[6])
            marker.pose.orientation.w = 1.0
            marker.pose.position.x = objects_pos[0]
            marker.pose.position.y = objects_pos[1]
            marker.pose.position.z = objects_pos[2] + 0.2
            markerArray.markers.append(marker)
            #for text specific id
            marker = Marker()
            objects_pos = list(sql_data.ix[i, ["position_x", "position_y", "position_z", "generic_id_0", "generic_score_0","generic_name_0", "specific_id_0"]])
            marker.id = i + num#"object_{0}".format(i)
            marker.header.frame_id = "/map"
            marker.type = marker.TEXT_VIEW_FACING #marker.SPHERE
            marker.action = marker.ADD
            marker.scale.x = 0.08 # objects_pos[4] / 10.0
            ...
            marker.color.g = color[objects_pos[6]][1]
            marker.color.b = color[objects_pos[6]][2]
            marker.text = objects_pos[5]
            marker.pose.orientation.w = 1.0
            marker.pose.position.x = objects_pos[0]
            marker.pose.position.y = objects_pos[1]
            marker.pose.position.z = objects_pos[2] + 0.1
            markerArray.markers.append(marker)
            #for points
            marker = Marker()
            objects_pos = list(sql_data.ix[i, ["position_x", "position_y", "position_z", "generic_id_0", "generic_score_0","generic_name_0", "specific_id_0"]])
            marker.id = i + num * 2#"object_{0}".format(i)
            marker.header.frame_id = "/map"
            if objects_pos[5] is "bottle":
                ...
                marker.action = marker.ADD
                marker.scale.x = 0.18 # objects_pos[4] / 10.0
                marker.scale.y = 0.18 # objects_pos[4] / 10.0
                marker.scale.z = 0.18 # objects_pos[4] / 10.0
                marker.color.a = 0.2 #objects_pos[4]
                marker.color.r = color[objects_pos[6]][0]
                marker.color.g = color[objects_pos[6]][1]
                marker.color.b = color[objects_pos[6]][2]
                marker.pose.orientation.w = 1.0
    ...
    def _near_point_deleate(self, data_frame, score_type=True):
        ...
        data_frame = data_frame.sort_values(by=score_name,ascending=False)
        N = len(data_frame)
        if N == 0:
            return data_frame
        df = data_frame[0:1]
...
if __name__ == '__main__':
    ...
    obj = Objects()
    while not rospy.is_shutdown():
        obj.get_object_num_from_time(10,0,0.7,True)
