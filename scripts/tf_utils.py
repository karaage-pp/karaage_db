...
class Tf(object):
    def __init__(self):
        #
        ns = "/tf2_buffer_server"
        self._buffer = tf2_ros.Buffer()
        self.lis = tf2_ros.TransformListener(self._buffer)
#        self._buffer = tf2_ros.BufferClient(ns)
#        if not self._buffer.wait_for_server(rospy.Duration(3)):
#            rospy.logwarn("wait for {ns}".format(**locals()))
#            self._buffer.wait_for_server()
